...
// 🆕 v13.6: parser JSON yang tahan terhadap file kosong / korup
async function safeReadJson(filePath, fallback) {
  try {
    ...
    console.error(`❌ Error parse ${filePath}: ${err.message} → fallback dipakai.`);
    try {
      await fsp.writeFile(filePath, JSON.stringify(fallback, null, 2), "utf8");
    } catch (_) {}
    return fallback;
...
async function loadUsers() {
  db = await safeReadJson(usersJson, {});
  if (typeof db !== "object" || Array.isArray(db) || db === null) {
    console.warn("⚠️ users.json bukan object — direset.");
    db = {};
  }
}
...
async function loadOwners() {
  dbOwner = await safeReadJson(ownerJson, []);
  if (!Array.isArray(dbOwner)) {
    console.warn("⚠️ owner.json bukan array — direset.");
    dbOwner = [];
  }
}
