...
// 🆕 v13.4: Flag global anti-init-ganda untuk semua cron Pusdat
global.__pusdatCronInited = global.__pusdatCronInited || false;
global.__pusdatDashboardInited = global.__pusdatDashboardInited || false;
global.__pusdatRateLimiterBound = global.__pusdatRateLimiterBound || false;
...
_Website: autoresbot.com/contact_`;
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      ...
      // ════════════════════════════════════════════════════
      // 🆕 v13.4 — ANTI-DUPLIKASI INIT
      // Init cron HANYA pada koneksi pertama. Reconnect berikutnya
      // hanya update referensi socket — TIDAK membuat cron baru.
      // ════════════════════════════════════════════════════
      if (folder === 'session') {
        if (!global.__pusdatCronInited) {
          console.log('[BOOT] 🚀 First connection — initializing all Pusdat cron jobs...');
          initCronBroadcast(sock);
          initCronProker(sock);
          global.__pusdatCronInited = true;
        } else {
          // Reconnect — cukup update socket saja
          console.log('[BOOT] 🔄 Reconnect detected — updating socket reference only (no re-init).');
          updateBroadcastSocket(sock);
          updateProkerSocket(sock);
        }

        if (!global.__pusdatDashboardInited) {
          startDashboard();
          global.__pusdatDashboardInited = true;
        }

        if (!global.__pusdatRateLimiterBound) {
          // 🛡️ FIX: config.js menyimpan list owner di `owner_number`, bukan `owner`.
          //         Tanpa fallback ini, getOwnerJids() di rateLimiter selalu kosong
          //         dan notif abuse tidak pernah terkirim ke owner.
          bindAppConfig({ owner: config.owner || config.owner_number || [] });
          global.__pusdatRateLimiterBound = true;
        }
      }
      ...
      if (config.autobackup) {
        ...
        try {
          ...
          // 🛡️ FIX: `remoteJid` & `message` TIDAK ada di scope connection.update.
          //         Sebelumnya kalau backup gagal → ReferenceError → bot crash.
          //         Sekarang kirim notif ke nomor bot sendiri (sumber yang valid).
          try {
            const ownerJid = `${config.phone_number_bot}@s.whatsapp.net`;
            await sock.sendMessage(ownerJid, {
              text: `❌ _Gagal melakukan backup:_ ${err?.message || err}`,
            });
          } catch (_) {
            // jangan biarkan notif gagal mengkrash bot
          }
        }
      }
