import { getSession as getGatewaySession } from '../../lib/dbAccess.js';
import fs from 'fs';
...
async function process(sock, messageInfo) {
  ...
  try {
    ...
    // === PATCH: Skip jika user sedang dalam session gateway ===
    const _isInGatewaySession = !!getGatewaySession(sender);
    if (fitur?.autoai && command !== 'on' && command !== 'off' && !prefix && !_isInGatewaySession) {
    ...
    if (fitur?.autosimi && command !== 'on' && command !== 'off' && !_isInGatewaySession) {
    ...
    if (fitur?.autorusuh && command !== 'on' && command !== 'off' && !_isInGatewaySession) {
      const isQuotedMessageFromBot = (() => {
        if (!isQuoted?.sender) return false;
